...
def pretendard_font_bytes() -> Optional[bytes]:
    if FONT.exists():
        return FONT.read_bytes()
    index = ROOT / "index.html"
    if index.exists():
        match = re.search(r"data:font/woff2;base64,([A-Za-z0-9+/=]+)", index.read_text(encoding="utf-8"))
        if match:
...
def svg_embedded_styles() -> str:
    face = font_face_css()
    if face:
        return (
            "<style>"
            + face
            + "text{font-family:Pretendard;letter-spacing:0}"
            + "</style>"
        )
    return SVG_TEXT_STYLE
...
FIGURE_FONT_SCALE = 0.98
_FONT_METRICS: Dict[int, Optional[Tuple[dict, object, float]]] = {}


def _font_metrics(font_weight: int) -> Optional[Tuple[dict, object, float]]:
    if font_weight in _FONT_METRICS:
        return _FONT_METRICS[font_weight]
    data = pretendard_font_bytes()
    if not data:
        _FONT_METRICS[font_weight] = None
        return None
    try:
    ...
    with tempfile.NamedTemporaryFile(suffix=".woff2", delete=False) as tmp:
        tmp.write(data)
        font_path = tmp.name
    font = instantiateVariableFont(TTFont(font_path), {"wght": font_weight})
    metrics = (font.getBestCmap(), font.getGlyphSet(), font["head"].unitsPerEm)
    _FONT_METRICS[font_weight] = metrics
    return metrics


def _line_width(line: str, size: float, *, font_weight: int = 400) -> float:
    metrics = _font_metrics(font_weight)
    if metrics is not None:
        cmap, glyph_set, units = metrics
        advance = 0.0
        for ch in line:
            glyph = cmap.get(ord(ch))
            if glyph:
                advance += glyph_set[glyph].width
        if advance:
            return advance * size / units
    return _estimate_line_width(line, size, font_weight=font_weight)
...
def _estimate_line_width(line: str, size: float, *, font_weight: int = 400) -> float:
    weight_scale = 1.08 if font_weight >= 900 else 1.03 if font_weight >= 700 else 1.0
    unit = size * 0.50 * weight_scale
    width = 0.0
    for ch in line:
        if ch == " ":
            width += unit * 0.42
...
def _wrap_text_lines(
    ...
    *,
    ...
    size = font_size * FIGURE_FONT_SCALE
    words = text.split()
    lines: List[str] = []
    current: List[str] = []
    current_w = 0.0
    for word in words:
        word_w = _line_width(word, size, font_weight=font_weight)
        space_w = _line_width(" ", size, font_weight=font_weight) if current else 0.0
        if space_w <= 0 and current:
            space_w = size * 0.28
        if current and current_w + space_w + word_w > max_line_width:
...
def _optimal_wrap_lines(
    ...
    *,
    ...
    word_min = _line_width(longest_word, size, font_weight=font_weight)
    full_w = _line_width(text, size, font_weight=font_weight)
    start = max(word_min, min_inner_width, full_w * 0.36)

    best_lines: List[str] | None = None
    best_width = max_inner_width
    best_score = float("inf")

    for inner in range(int(start), int(max_inner_width) + 1, 4):
        ...
        if not lines or len(lines) > max_lines:
            continue
        content_w = max(_line_width(line, size, font_weight=font_weight) for line in lines)
        if content_w > inner + 1:
            continue
        last_ratio = len(lines[-1]) / max(len(lines[0]), 1)
        orphan_penalty = 180 if len(lines) > 1 and last_ratio < 0.38 else 0
        line_balance = max(len(line) for line in lines) - min(len(line) for line in lines)
        score = (
            content_w
            + orphan_penalty
            + line_balance * 0.35
            + (len(lines) - 1) * 28
            + max(0, len(lines) - 3) * 60
        )
        if score < best_score:
            best_score = score
            best_lines = lines
            best_width = content_w

    if best_lines is None:
        best_lines = _wrap_text_lines(
            text,
            max_inner_width,
            font_size,
            font_weight=font_weight,
        )
        best_width = max(_line_width(line, size, font_weight=font_weight) for line in best_lines)
    return best_lines, best_width


def svg_wrapped_text_box(
    parts: List[str],
    *,
    cx: float,
    bottom: float,
    ...
    pad_x: float = 10,
    pad_y: float = 6,
    line_height_factor: float = 1.36,
) -> None:
    size = font_size * FIGURE_FONT_SCALE
    max_inner = max_width - 2 * pad_x
    longest = max(text.split(), key=len)
    word_min = _line_width(longest, size, font_weight=font_weight)
    lines, content_w = _optimal_wrap_lines(
        text,
        font_size=font_size,
        font_weight=font_weight,
        min_inner_width=word_min,
        max_inner_width=max_inner,
        max_lines=4,
    )
    line_h = size * line_height_factor
    line_widths = [_line_width(line, size, font_weight=font_weight) for line in lines]
    content_w = max(line_widths)
    box_w = content_w + 2 * pad_x
    box_h = len(lines) * line_h + 2 * pad_y
    box_x = cx - box_w / 2
    box_y = bottom - box_h
    radius = min(box_h / 2, 17)
    text_x = box_x + pad_x
    parts.append(
    ...
    for i, line in enumerate(lines):
        ty = box_y + pad_y + (i + 0.78) * line_h
        parts.append(
            f'<text x="{text_x:.1f}" y="{ty:.1f}" text-anchor="start" font-size="{font_size}" '
            f'font-weight="{font_weight}" fill="{color}">{esc(line)}</text>'
        )
...
def transfer_svg() -> str:
    ...
    parts.append('<path d="M64 404H804" stroke="#c5d9e8" stroke-width="2"/>')
    parts.append('</svg>')
    return scaled_figure_svg("".join(parts))
...
def semantic_svg() -> str:
    ...
    parts.append('<style>text{font-family:Arial, Helvetica, sans-serif;letter-spacing:0}</style>')
    ...
    parts.append('<text x="219" y="86" text-anchor="middle" font-size="15" fill="#000000">generic and fine meanings share one surface</text>')
    ...
    parts.append('<path d="M116 166L158 246M300 150L334 226M142 314L286 318" fill="none" stroke="#8f8378" stroke-width="2.1" stroke-linecap="round" opacity=".72"/>')
    parts.append('<path d="M116 166C178 190 238 158 300 150M116 166C144 232 136 276 142 314" fill="none" stroke="#bbaea0" stroke-width="2.4" stroke-dasharray="7 8"/>')
    for x, y, k, fill in [(116,166,"T","#9fb7d9"),(158,246,"I","#78a99a"),(300,150,"T","#9fb7d9"),(334,226,"I","#78a99a"),(142,314,"T","#9fb7d9"),(286,318,"I","#78a99a")]:
        node(x, y, k, fill)
    pill(104, 112, "text: dog", "#eef3f8", "#cfdae8", size=13)
    pill(318, 112, "text: yellow dog", "#eef3f8", "#cfdae8", size=12)
    pill(110, 340, "text: brown dog", "#eef3f8", "#cfdae8", size=12)
    pill(214, 250, "generic dog image", "#e7f0e9", "#cbdccd", size=12)
    pill(330, 248, "yellow dog image", "#e7f0e9", "#cbdccd", size=12)
    pill(310, 340, "brown dog image", "#e7f0e9", "#cbdccd", size=12)
    parts.append('<rect x="34" y="390" width="370" height="42" rx="14" fill="#f6e7dc" stroke="#e6cfc1"/>')
    parts.append('<text x="219" y="407" text-anchor="middle" font-size="15" font-weight="900" fill="#000000">Flat angular separation</text>')
    parts.append('<text x="219" y="424" text-anchor="middle" font-size="14" fill="#000000">generic dog and fine dog captions stay separate</text>')
    parts.append('</g>')
    ...
    parts.append('<rect x="24" y="189" width="96" height="20" rx="10" fill="#eef3f8" stroke="#cfdae8"/>')
    parts.append('<text x="72" y="203" text-anchor="middle" font-size="9.6" font-weight="850" fill="#000000">Lorentz model</text>')
    parts.append('</g>')
    ...
    parts.append('<text x="235" y="42" text-anchor="middle" font-size="20" font-weight="900" fill="#000000">Hyperbolic space</text>')
    parts.append('<text x="235" y="66" text-anchor="middle" font-size="15" fill="#000000">project Euclidean embeddings onto Lorentz space</text>')
    parts.append('<text x="235" y="86" text-anchor="middle" font-size="15" fill="#000000">caption hierarchy: category to specific attributes</text>')
    parts.append('<path d="M235 95V382" stroke="#c8baad" stroke-width="2" stroke-dasharray="6 7"/>')
    parts.append('<text x="266" y="375" font-size="14" font-weight="400" fill="#6f7882">time-like axis</text>')
    ...
    parts.append('<path d="M235 350C218 298 199 246 178 200C164 170 150 144 132 118" fill="none" stroke="#e5dad0" stroke-width="2"/>')
    parts.append('<path d="M235 350C252 298 275 244 300 190C318 152 336 128 356 112" fill="none" stroke="#e5dad0" stroke-width="2"/>')
    parts.append('<path d="M235 346C225 302 210 256 190 212C174 176 154 144 132 118" fill="none" stroke="url(#rangeLine)" stroke-width="14" stroke-linecap="round" opacity=".62"/>')
    parts.append('<path d="M235 346C252 300 278 247 302 195C320 157 337 133 355 112" fill="none" stroke="url(#rangeLine)" stroke-width="14" stroke-linecap="round" opacity=".62"/>')
    parts.append('<path d="M142 124C200 106 270 106 344 124" fill="none" stroke="#d49a8c" stroke-width="7" stroke-linecap="round" stroke-dasharray="2 17" opacity=".52"/>')
    parts.append('<path d="M382 116C352 116 332 118 304 126" fill="none" stroke="#d49a8c" stroke-width="2" stroke-linecap="round" opacity=".8"/>')
    parts.append('<rect x="374" y="96" width="82" height="42" rx="14" fill="#f6e7dc" stroke="#e6cfc1" opacity=".96"/>')
    parts.append('<text x="415" y="113" text-anchor="middle" font-size="11.5" font-weight="900" fill="#000000">residual</text>')
    parts.append('<text x="415" y="129" text-anchor="middle" font-size="11.5" font-weight="900" fill="#000000">subspace</text>')
    parts.append('<path d="M382 285C338 272 309 242 278 211" fill="none" stroke="#78a99a" stroke-width="2.4" stroke-linecap="round" opacity=".86"/>')
    parts.append('<rect x="362" y="264" width="94" height="42" rx="14" fill="#e7f0e9" stroke="#cbdccd" opacity=".96"/>')
    parts.append('<text x="409" y="281" text-anchor="middle" font-size="11.5" font-weight="900" fill="#000000">dominant</text>')
    parts.append('<text x="409" y="297" text-anchor="middle" font-size="11.5" font-weight="900" fill="#000000">range rank</text>')
    parts.append('<circle cx="235" cy="350" r="22" fill="#f2dfaa" stroke="#ffffff" stroke-width="5"/>')
    parts.append('<text x="235" y="346" text-anchor="middle" font-size="14" fill="#000000"><tspan font-weight="950">generic</tspan></text>')
    parts.append('<text x="235" y="362" text-anchor="middle" font-size="13" fill="#000000">dog</text>')
    for x, y, k, fill in [(176,214,"T","#9fb7d9"),(144,188,"I","#78a99a"),(306,160,"T","#9fb7d9"),(348,132,"I","#78a99a")]:
        node(x, y, k, fill)
    parts.append('<path d="M176 214L144 188M306 160L348 132" stroke="#8f8378" stroke-width="2.2" stroke-linecap="round"/>')
    parts.append('<rect x="24" y="236" width="178" height="42" rx="14" fill="#eef3f8" stroke="#cfdae8"/>')
    parts.append('<text x="113" y="253" text-anchor="middle" font-size="11.5" fill="#000000"><tspan font-weight="950">fine-grained</tspan>: yellow dog</text>')
    parts.append('<text x="113" y="269" text-anchor="middle" font-size="11.5" font-weight="850" fill="#000000">image-text pair</text>')
    parts.append('<rect x="262" y="180" width="178" height="42" rx="14" fill="#eef3f8" stroke="#cfdae8"/>')
    parts.append('<text x="351" y="197" text-anchor="middle" font-size="11.5" fill="#000000"><tspan font-weight="950">fine-grained</tspan>: brown dog</text>')
    parts.append('<text x="351" y="213" text-anchor="middle" font-size="11.5" font-weight="850" fill="#000000">image-text pair</text>')
    ...
    parts.append('<text x="75" y="22" text-anchor="middle" font-size="14" font-weight="900" fill="#000000">hierarchy view</text>')
    ...
    parts.append('<circle cx="54" cy="62" r="5" fill="#9fb7d9"/><text x="28" y="48" font-size="12" font-weight="900" fill="#000000">fine</text>')
    parts.append('<circle cx="96" cy="62" r="5" fill="#9fb7d9"/><text x="99" y="48" font-size="12" font-weight="950" fill="#000000">generic</text>')
    parts.append('<rect x="70" y="80" width="10" height="10" rx="3" fill="#78a99a"/><text x="84" y="91" font-size="12" font-weight="800" fill="#000000">image</text>')
    parts.append('</g>')
    pill(120, 424, "category range rank", "#e7f0e9", "#cbdccd", size=13)
    pill(336, 424, "attribute residual subspace", "#f6e7dc", "#e6cfc1", size=13)
    parts.append('</g>')
    ...
    parts.append('<rect x="584" y="596" width="570" height="58" rx="14" fill="#e7f0e9" stroke="#cbdccd"/>')
    parts.append('<text x="869" y="617" text-anchor="middle" font-size="14" font-weight="900" fill="#000000">Text is hierarchical: <tspan font-weight="950">generic</tspan> dog aligns broadly.</text>')
    parts.append('<text x="869" y="636" text-anchor="middle" font-size="14" font-weight="900" fill="#000000">Yellow and brown dog pairs stay fine-grained and specific.</text>')
    parts.append('</svg>')
    return scaled_figure_svg("".join(parts))
...
def render_page(page: int, out: Path) -> bool:
    if not PDF.exists() or not shutil.which("gs"):
        return False
    return run([
        "gs", "-dNOSAFER", "-dNOPAUSE", "-dBATCH", "-sDEVICE=pngalpha", f"-r{RENDER_DPI}",
...
def make_png_crops() -> Dict[str, Path]:
    ...
    if not page29.exists() and Path("/tmp/raha_page_29.png").exists():
        page29.write_bytes(Path("/tmp/raha_page_29.png").read_bytes())
    if not page26.exists() and Path("/tmp/raha_app_01.png").exists():
        page26.write_bytes(Path("/tmp/raha_app_01.png").read_bytes())

    scale = RENDER_DPI / BASE_DPI
    out = {}
    ablation = GEN / "fig2_ablation.png"
    if crop(page14, scale_geom("780x240+295+228", scale), ablation, trim=False, chart_trim=False):
        out["ablation"] = ablation
    page30 = GEN / "page_30.png"
    if not page30.exists():
        render_page(30, page30)
    fig_a6_ab = GEN / "fig_a6_ab.png"
    if crop(page30, scale_geom("416x200+264+235", scale), fig_a6_ab, trim=False, chart_trim=False, border_trim=True):
        out["fig_a6_ab"] = fig_a6_ab
    fig_a6_cd = GEN / "fig_a6_cd.png"
    if crop(page30, scale_geom("416x210+680+235", scale), fig_a6_cd, trim=False, chart_trim=False, border_trim=True):
        out["fig_a6_cd"] = fig_a6_cd
    fig_a6 = GEN / "fig_a6_sensitivity.png"
    if crop(page30, scale_geom("832x200+264+235", scale), fig_a6, trim=False, chart_trim=False):
        out["fig_a6"] = fig_a6
    main = GEN / "main_qualitative.png"
    if crop(page12, scale_geom("785x230+292+606", scale), main):
        out["main_qual"] = main
    for key, _label, geometry in APPENDIX_CROPS:
        target = GEN / f"qual_{key}.png"
        if crop(page26, scale_geom(geometry, scale), target):
            out[f"qual_{key}"] = target
    raha_crops = [
        ("cifar", "280x256+830+226"),
        ("cub", "280x216+830+570"),
        ("cars", "280x218+830+848"),
        ("imagenet", "280x228+830+1124"),
    ]
    for key, geometry in raha_crops:
        target = GEN / f"qual_{key}_raha.png"
        if crop(page26, scale_geom(geometry, scale), target):
            out[f"qual_{key}_raha"] = target
    res_dir = ROOT / "res"
    ...
    for name in (
        ...
        "fig_a6_ab.png",
        "fig_a6_cd.png",
        "fig_a6_sensitivity.png",
        "main_qualitative.png",
        "qual_cifar.png",
        "qual_cub.png",
        "qual_cars.png",
        "qual_imagenet.png",
        ...
        "qual_cub_raha.png",
        "qual_cars_raha.png",
        "qual_imagenet_raha.png",
    ):
        src = GEN / name
        if src.exists():
            shutil.copy2(src, res_dir / name)
    return out
...
def make_tables() -> Tuple[str, str, str]:
    ...
    for dataset, series in RETRIEVAL.items():
        ...
        for budget in [100, 200, 500, 1000]:
            ...
            dataset_cell = f"<td rowspan=\"4\" class=\"dataset-cell\"><span class=\"dataset-pill {dataset_class}\">{esc(dataset)}</span></td>" if budget == 100 else ""
            retrieval_rows.append(
                f"<tr class=\"{dataset_class} {pair_class}\">{dataset_cell}<td><span class=\"pair-pill {pair_class}\">{budget}</span></td><td>{b:.1f}</td><td>{r:.1f}</td><td class=\"delta {sign}\">{delta:+.1f}</td></tr>"
            )
...
def setup_svg() -> str:
    parts: List[str] = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 500" role="img">']
    parts.append(svg_embedded_styles())
    parts.append('<defs><marker id="setupArrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#1d76bb"/></marker></defs>')
    parts.append('<rect width="1080" height="500" rx="24" fill="#ffffff"/>')
    parts.append('<rect x="22" y="22" width="1036" height="456" rx="20" fill="#fbfdff" stroke="#c5d9e8"/>')
    parts.append('<text x="540" y="62" text-anchor="middle" font-size="31" font-weight="900" fill="#000000">Vision-language dataset distillation setup</text>')
    parts.append('<text x="540" y="91" text-anchor="middle" font-size="16" fill="#000000">Compress real image-text data into a compact learned set for efficient vision-language training.</text>')

    card_y, card_h = 124, 286
    divider_y = 248

    def card_frame(x: int, w: int, title: str, lines: List[str], fill: str = '#ffffff') -> None:
        parts.append(f'<rect x="{x}" y="{card_y}" width="{w}" height="{card_h}" rx="18" fill="{fill}" stroke="#c5d9e8"/>')
        parts.append(f'<text x="{x+w/2}" y="158" text-anchor="middle" font-size="19" font-weight="900" fill="#000000">{esc(title)}</text>')
        for i, line in enumerate(lines):
            parts.append(f'<text x="{x+w/2}" y="{186+i*22}" text-anchor="middle" font-size="14" fill="#000000">{esc(line)}</text>')
        parts.append(f'<line x1="{x+24}" y1="{divider_y}" x2="{x+w-24}" y2="{divider_y}" stroke="#d6ebf8" stroke-width="1.4"/>')

    card_frame(50, 260, 'Input', ['Large real paired dataset', 'many image-text examples', 'rich but costly to train'], '#ffffff')
    card_frame(390, 300, 'RAHA distillation', ['Learn synthetic pixels', 'learn text embeddings', 'match cross-relevance'], '#f7f1e8')
    card_frame(770, 260, 'Output', ['Small synthetic dataset', 'reused for training', 'keeps downstream utility'], '#ffffff')

    parts.append('<path d="M326 252H374" stroke="#1d76bb" stroke-width="4" stroke-linecap="round" marker-end="url(#setupArrow)"/>')
    parts.append('<text x="350" y="234" text-anchor="middle" font-size="12" font-weight="850" fill="#000000">distill</text>')
    parts.append('<path d="M706 252H754" stroke="#1d76bb" stroke-width="4" stroke-linecap="round" marker-end="url(#setupArrow)"/>')
    parts.append('<text x="730" y="234" text-anchor="middle" font-size="12" font-weight="850" fill="#000000">reuse</text>')

    for row, y in enumerate([268, 312, 356]):
        for col, x in enumerate([82, 194]):
            opacity = 0.56 + ((row + col) % 2) * 0.26
            parts.append(f'<rect x="{x}" y="{y}" width="24" height="20" rx="5" fill="#76a99a" opacity="{opacity:.2f}"/>')
            parts.append(f'<rect x="{x+30}" y="{y+2}" width="68" height="16" rx="8" fill="#eef3f8" stroke="#cfdae8"/>')
    parts.append('<text x="180" y="392" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">real images + real text</text>')

    parts.append('<circle cx="474" cy="306" r="34" fill="#e7f0e9" stroke="#cbdccd"/>')
    parts.append('<text x="474" y="313" text-anchor="middle" font-size="20" font-weight="900" fill="#000000">I</text>')
    parts.append('<circle cx="574" cy="306" r="34" fill="#eef3f8" stroke="#cfdae8"/>')
    parts.append('<text x="574" y="313" text-anchor="middle" font-size="20" font-weight="900" fill="#000000">T</text>')
    parts.append('<path d="M511 306H535" stroke="#1d76bb" stroke-width="3" stroke-linecap="round" marker-end="url(#setupArrow)"/>')
    parts.append('<path d="M615 305C646 290 660 290 678 305" fill="none" stroke="#1d76bb" stroke-width="3" stroke-linecap="round" marker-end="url(#setupArrow)"/>')
    parts.append('<text x="540" y="392" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">optimize compact image-text pairs</text>')

    output_card_x, output_card_w = 770, 260
    output_card_cx = output_card_x + output_card_w / 2
    img_w, img_h, gap, pill_w, pill_h = 24, 22, 6, 50, 16
    pair_w = img_w + gap + pill_w
    col_centers = [output_card_cx - pair_w / 2 - 10, output_card_cx + pair_w / 2 + 10]
    row_ys = [268, 314]
    output_pairs = [
        (col_centers[0], row_ys[0], '#76a99a'),
        (col_centers[1], row_ys[0], '#9fb7d9'),
        (col_centers[0], row_ys[1], '#ead48f'),
        (col_centers[1], row_ys[1], '#bfd4a7'),
    ]
    for cx, y, fill in output_pairs:
        x = cx - pair_w / 2
        parts.append(
            f'<rect x="{x:.1f}" y="{y}" width="{img_w}" height="{img_h}" rx="6" fill="{fill}" '
            f'opacity=".92" stroke="#ffffff" stroke-width="2"/>'
        )
        parts.append(
            f'<rect x="{x + img_w + gap:.1f}" y="{y + 3}" width="{pill_w}" height="{pill_h}" '
            f'rx="8" fill="#eef3f8" stroke="#cfdae8"/>'
        )
    parts.append('<text x="900" y="388" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">learned images + learned text embeddings</text>')

    svg_wrapped_text_box(
        parts,
        cx=540,
        bottom=472,
        text="Train the same vision-language model faster, then evaluate retrieval, transfer, and prompted classification.",
        max_width=560,
        font_size=14,
    )
    parts.append('</svg>')
...
def method_walkthrough_svg() -> str:
    parts: List[str] = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1120 620" role="img">']
    parts.append(SVG_TEXT_STYLE)
    parts.append('<defs><marker id="walkArrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="8" markerHeight="8" orient="auto"><path d="M0 0L10 5L0 10Z" fill="#1d76bb"/></marker><linearGradient id="walkGrad" x1="0" x2="1"><stop offset="0" stop-color="#e7f0e9"/><stop offset="1" stop-color="#eef3f8"/></linearGradient></defs>')
    parts.append('<rect width="1120" height="620" rx="24" fill="#ffffff"/>')
    parts.append('<rect x="22" y="22" width="1076" height="576" rx="20" fill="#fbfdff" stroke="#c5d9e8"/>')
    parts.append('<text x="560" y="62" text-anchor="middle" font-size="30" font-weight="900" fill="#000000">RAHA walkthrough in one distillation step</text>')
    parts.append('<text x="560" y="91" text-anchor="middle" font-size="15" fill="#000000">A real batch defines cross-modal relevance targets, while learned synthetic image-text pairs are the only optimized data.</text>')
    ...
    def card(x: int, title: str, lines: List[str], fill: str) -> None:
        parts.append(f'<rect x="{x}" y="{card_y}" width="{card_w}" height="{card_h}" rx="16" fill="{fill}" stroke="#c5d9e8"/>')
        parts.append(f'<text x="{x + card_w / 2:.0f}" y="158" text-anchor="middle" font-size="16" font-weight="900" fill="#000000">{esc(title)}</text>')
        for i, line in enumerate(lines):
            parts.append(f'<text x="{x + card_w / 2:.0f}" y="{188 + i * 21}" text-anchor="middle" font-size="13" fill="#000000">{esc(line)}</text>')
        parts.append(f'<line x1="{x + 18}" y1="228" x2="{x + card_w - 18}" y2="228" stroke="#d8e5ee"/>')

    xs = [40, 252, 464, 676, 888]
    card(xs[0], '1. Pair batches', ['real batch = target', 'synthetic set = learned'], '#ffffff')
    for i, y in enumerate([visual_y + 6, visual_y + 52, visual_y + 98]):
        color = ['#76a99a', '#9fb7d9', '#ead48f'][i]
        parts.append(f'<rect x="72" y="{y}" width="34" height="27" rx="7" fill="{color}" stroke="#ffffff" stroke-width="3"/>')
        parts.append(f'<rect x="118" y="{y + 3}" width="82" height="21" rx="10.5" fill="#eef3f8" stroke="#cfdae8"/>')
        parts.append(f'<line x1="108" y1="{y + 14}" x2="118" y2="{y + 14}" stroke="#94a8b9" stroke-width="2"/>')
    parts.append('<rect x="68" y="406" width="136" height="36" rx="18" fill="#e7f0e9" stroke="#cbdccd"/>')
    parts.append('<text x="136" y="429" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">image-text pairs</text>')

    card(xs[1], '2. Hyperbolic lift', ['map embeddings to', 'Lorentz geometry'], '#f7f1e8')
    parts.append('<path d="M302 258C329 300 339 352 348 390C357 352 367 300 394 258Z" fill="#eef3f8" stroke="#b9ccdc"/>')
    parts.append('<ellipse cx="348" cy="268" rx="46" ry="10" fill="none" stroke="#d9cec1"/>')
    parts.append('<ellipse cx="348" cy="326" rx="28" ry="8" fill="none" stroke="#d9cec1"/>')
    parts.append('<circle cx="330" cy="294" r="8" fill="#76a99a" stroke="#ffffff" stroke-width="3"/>')
    parts.append('<circle cx="365" cy="338" r="8" fill="#9fb7d9" stroke="#ffffff" stroke-width="3"/>')
    parts.append('<rect x="286" y="406" width="124" height="36" rx="18" fill="#eef3f8" stroke="#c5d9e8"/>')
    parts.append('<text x="348" y="429" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">hierarchy aware</text>')

    card(xs[2], '3. Decompose', ['SVD separates', 'range and residual'], '#ffffff')
    for r in range(5):
        for c in range(5):
            opacity = .95 if r == c or abs(r - c) == 1 else .28
            parts.append(f'<rect x="518" y="260" width="18" height="18" rx="3" fill="#9fb7d9" opacity="{opacity}" transform="translate({c * 22},{r * 22})"/>')
    parts.append('<path d="M522 392H598" stroke="#76a99a" stroke-width="6" stroke-linecap="round"/>')
    parts.append('<path d="M522 414H598" stroke="#d49a8c" stroke-width="5" stroke-linecap="round" stroke-dasharray="5 7"/>')
    parts.append('<text x="610" y="397" font-size="12.5" font-weight="850" fill="#000000">range</text>')
    parts.append('<text x="610" y="419" font-size="12.5" font-weight="850" fill="#000000">residual</text>')

    card(xs[3], '4. Match relevance', ['match dominant rank', 'regularize remainder'], '#f7f1e8')
    parts.append('<path d="M716 330C748 294 792 294 824 330" fill="none" stroke="#76a99a" stroke-width="8" stroke-linecap="round" opacity=".82"/>')
    parts.append('<path d="M716 376C748 396 792 352 824 378" fill="none" stroke="#d49a8c" stroke-width="5.5" stroke-linecap="round" opacity=".7" stroke-dasharray="5 8"/>')
    parts.append('<rect x="706" y="406" width="128" height="36" rx="18" fill="#e7f0e9" stroke="#cbdccd"/>')
    parts.append('<text x="770" y="429" text-anchor="middle" font-size="13" font-weight="850" fill="#000000">real vs synthetic</text>')

    card(xs[4], '5. Update synthetic set', ['optimize N images', 'N text embeddings'], '#ffffff')
    for i, y in enumerate([260, 306, 352]):
        color = ['#76a99a', '#9fb7d9', '#ead48f'][i]
        parts.append(f'<rect x="926" y="{y}" width="38" height="30" rx="8" fill="{color}" stroke="#ffffff" stroke-width="3"/>')
        parts.append(f'<rect x="976" y="{y + 5}" width="68" height="20" rx="10" fill="#eef3f8" stroke="#cfdae8"/>')
        parts.append(f'<line x1="966" y1="{y + 15}" x2="976" y2="{y + 15}" stroke="#94a8b9" stroke-width="2"/>')
    parts.append('<text x="945" y="333" text-anchor="middle" font-size="13" font-weight="900" fill="#000000">I</text>')
    parts.append('<text x="1010" y="333" text-anchor="middle" font-size="13" font-weight="900" fill="#000000">T</text>')
    parts.append('<text x="1058" y="335" text-anchor="middle" font-size="14" font-weight="900" fill="#000000">×N</text>')
    parts.append('<path d="M1054 264C1072 292 1072 342 1054 370" fill="none" stroke="#1d76bb" stroke-width="3" stroke-linecap="round" marker-end="url(#walkArrow)"/>')
    parts.append('<rect x="916" y="400" width="152" height="48" rx="18" fill="#eef3f8" stroke="#c5d9e8"/>')
    parts.append('<text x="992" y="420" text-anchor="middle" font-size="12.5" font-weight="850" fill="#000000">N synthetic</text>')
    parts.append('<text x="992" y="438" text-anchor="middle" font-size="12.5" font-weight="850" fill="#000000">image-text pairs</text>')

    for x in [226, 438, 650, 862]:
        parts.append(f'<path d="M{x} 304H{x + 22}" stroke="#1d76bb" stroke-width="3.2" stroke-linecap="round" marker-end="url(#walkArrow)"/>')

    parts.append('<rect x="126" y="522" width="868" height="50" rx="18" fill="url(#walkGrad)" stroke="#cbdccd"/>')
    parts.append('<text x="560" y="543" text-anchor="middle" font-size="14" font-weight="900" fill="#000000">Real batches define target image-text relevance.</text>')
    parts.append('<text x="560" y="563" text-anchor="middle" font-size="14" font-weight="850" fill="#000000">Synthetic pairs are updated to reproduce it with far fewer pairs while preserving downstream utility.</text>')
    parts.append('</svg>')
    return scaled_figure_svg(''.join(parts))
...
def build() -> None:
    ...
    svgs = {
        ...
        "setup": setup_svg(),
        "walkthrough": method_walkthrough_svg(),
    }
    ...
    replacements = {
        ...
        "%%SETUP_FIG%%": svg_uri(svgs["setup"]),
        "%%WALKTHROUGH_FIG%%": svg_uri(svgs["walkthrough"]),
        ...
        "%%FIG_A6_AB%%": pngs["fig_a6_ab"].relative_to(ROOT).as_posix() if "fig_a6_ab" in pngs else svg_uri(placeholder_svg("Fig. A6 (a,b) crop unavailable")),
        "%%FIG_A6_CD%%": pngs["fig_a6_cd"].relative_to(ROOT).as_posix() if "fig_a6_cd" in pngs else svg_uri(placeholder_svg("Fig. A6 (c,d) crop unavailable")),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    h1 { letter-spacing: 0; }
    .paper-title { width: 100%; max-width: none; min-width: 0; font-size: 3.72rem; line-height: 1.02; margin: 0 0 18px; font-weight: 900; text-align: center; }
    ...
    .motivation-copy { display: grid; gap: 14px; }
    .motivation-copy .lead { margin: 0; }
    .setup-figure { margin-top: 22px; }
    .rq-block { margin-top: 22px; }
    .rq-heading { font-size: 1.06rem; margin: 0 0 10px; font-weight: 900; }
    .rq-response-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; }
    .rq-response { padding: 14px; background: #ffffff; border: 1px solid rgba(29,118,187,.16); border-radius: 8px; box-shadow: 6px 7px 0 rgba(29,118,187,.055), 0 10px 24px rgba(29,118,187,.07); }
    .rq-response h3 { margin-bottom: 6px; font-size: 1.02rem; }
    .rq-response p { margin: 0; color: var(--muted); font-size: .94rem; line-height: 1.45; }
    .method-walkthrough { margin-top: 18px; }
    .method-support { display: grid; grid-template-columns: minmax(0, 1.1fr) minmax(300px, .9fr); gap: 16px; margin-top: 18px; align-items: stretch; }
    .method-support .equation-card { margin-top: 0; display: grid; place-items: center; min-height: 100%; }
    .equation-legend { display: grid; gap: 9px; padding: 18px; background: #ffffff; border: 1px solid rgba(29,118,187,.16); border-radius: 8px; box-shadow: var(--shadow); }
    .equation-legend p { margin: 0; color: var(--muted); font-size: .96rem; line-height: 1.45; }
    .equation-legend b { color: var(--ink); font-weight: 900; }
    ...
    .classification-layout { grid-template-columns: minmax(0, 1.45fr) minmax(280px, .55fr); align-items: start; }
    ...
    .equation { font-family: var(--font-root); font-size: 2.02rem; font-style: normal; font-weight: 400; line-height: 1.32; color: var(--ink); }
    .equation sub { font-size: .56em; vertical-align: -0.32em; font-weight: 400; color: #000000; }
    .equation .lambda { font-family: var(--font-root); font-style: italic; font-weight: 400; }
    ...
    .retrieval-table .dataset-cell { vertical-align: middle; }
    ...
    .analysis-layout { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 16px; align-items: start; }
    ...
    @media (max-width: 980px) { .hero-grid, .split, .grid-2, .grid-3, .figure-table-grid, .figure-table-grid.reverse, .classification-layout, .method-support { grid-template-columns: 1fr; } .method-strip, .rq-response-grid { grid-template-columns: 1fr; } .qual-grid, .analysis-grid { grid-template-columns: 1fr; } .analysis-layout { grid-template-columns: 1fr; } .classification-samples-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } .paper-title { font-size: 2.72rem; } h2 { font-size: 2.65rem; } }
    @media (max-width: 760px) { body { font-size: 16px; } .nav-inner { flex-direction: column; align-items: flex-start; padding: 12px 0; } .wrap, .nav-inner { width: min(var(--max), calc(100% - 24px)); } .hero { padding-top: 42px; } .hero-visual { padding: 7px; } .citation-box { align-items: start; flex-direction: column; } table { min-width: 620px; } .classification-layout table, .transfer-layout table { min-width: 0; } .transfer-table th, .transfer-table td { padding: 7px 4px; font-size: .72rem; } .transfer-table th { font-size: .56rem; letter-spacing: 0; } .paper-title { font-size: 2.12rem; } h2 { font-size: 2.15rem; } .equation { font-size: 1.44rem; } }
  </style>
  ...
  <main id="top">
    ...
    <section class="band" id="abstract">
      <div class="wrap">
        ...
        <div class="figure setup-figure fade"><img src="%%SETUP_FIG%%" alt="Vision-language dataset distillation setup showing a large real image-text dataset compressed into a compact learned synthetic set."><div class="caption">Setup: RAHA learns a small synthetic image-text dataset from a large real paired dataset. The compact output is reused to train vision-language models while preserving retrieval, transfer, and classification utility.</div></div>
      </div>
    </section>
        ...
        <div class="split">
          <div class="section-head fade">
            ...
            <div class="motivation-copy">
              ...
              <p class="lead">This leads to three desiderata for VLDD: preserve the bidirectional relevance that trains retrieval models, retain the generic-to-specific hierarchy carried by text, and allocate limited synthetic capacity to the low-rank shared signal shown by LoRS rather than matching every feature direction uniformly. RAHA turns these desiderata into hyperbolic lifting, range-residual decomposition, and dominant cross-relevance matching.</p>
            </div>
          </div>
          <div class="figure fade"><img src="%%MOTIVATION_FIG%%" alt="Motivation diagram showing LoRS and MERU feeding into RAHA."><div class="caption"><strong>Conceptual connection to RAHA:</strong><br>Rank-aware relation structure and hyperbolic semantic geometry are combined for image-text dataset distillation.</div></div>
        </div>
        <div class="rq-block fade">
          <h3 class="rq-heading">Research Questions:</h3>
          <div class="rq-response-grid">
            <article class="rq-response"><h3>What must compression preserve?</h3><p>A distilled image-text set is useful only if a model trained on it learns the same bidirectional ranking behavior as from real data. This makes cross-modal relevance the core information capacity to preserve, beyond visual appearance alone.</p></article>
            <article class="rq-response"><h3>How should text hierarchy be retained?</h3><p>The relevance signal is not flat because captions move from generic categories to fine attributes and relations. This calls for geometry that keeps generic text broad while keeping fine-grained image-text pairs distinct.</p></article>
            <article class="rq-response"><h3>Where should capacity be spent?</h3><p>LoRS shows that useful image-text relations are concentrated in low-rank structure. RAHA therefore separates range and residual subspaces, matching dominant shared semantics while regulating weaker residual variation.</p></article>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="band dark" id="method">
      <div class="wrap">
        <div class="section-head fade">
          ...
          <h2>Lift hyperbolically, Decompose, then Match dominant cross-relevance.</h2>
        ...
        <div class="method-support fade">
          <div class="equation-card">
            <div class="equation">ℒ<sub>total</sub> = ℒ<sub>hITC</sub> + <span class="lambda">𝜆</span><sub>range</sub>ℒ<sub>range</sub> + <span class="lambda">𝜆</span><sub>residual</sub>ℒ<sub>residual</sub></div>
          </div>
          <div class="equation-legend">
            <p><b>ℒ<sub>hITC</sub></b> aligns synthetic image-text pairs after the Lorentz hyperbolic lift.</p>
            <p><b>ℒ<sub>range</sub></b> matches the dominant low-rank cross-relevance structure from real pairs.</p>
            <p><b>ℒ<sub>residual</sub></b> controls weaker residual correlations so the compact set does not spend capacity on noise.</p>
            <p><b>𝜆<sub>range</sub>, 𝜆<sub>residual</sub></b> balance semantic range preservation against residual refinement.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="band" id="analysis">
      <div class="wrap">
        <div class="section-head fade">
          ...
          <p class="lead">Component-level analysis shows RAHA is strongest when hyperbolic contrastive alignment is paired with dominant range supervision, controlled residual refinement, and hyperbolic rather than Euclidean relevance matching.</p>
        </div>
        <div class="analysis-layout">
          <div class="figure fade analysis-fig2"><div class="media"><img src="%%ABLATION_FIG%%" alt="Fig. 2 component ablation chart."></div><div class="caption">Fig. 2: Component attribution under the same protocol. Range matching provides the main relevance-transfer gain, while residual control improves the full model as a refinement when anchored by the range.</div></div>
          <div class="analysis-a6-column">
            <div class="figure fade analysis-a6-panel"><div class="media"><img src="%%FIG_A6_AB%%" alt="Fig. A6 panels (a) and (b)."></div></div>
            <div class="figure fade analysis-a6-bottom"><div class="media"><img src="%%FIG_A6_CD%%" alt="Fig. A6 panels (c) and (d)."></div><div class="caption">Fig. A6: Sensitivity and ablation analysis for matching terms, geometry, and residual control.</div></div>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="band" id="transfer">
      <div class="wrap">
        ...
        <div class="figure-table-grid reverse transfer-layout">
          ...
          <div class="figure fade"><img src="%%TRANSFER_FIG%%" alt="Cross-architecture transfer chart for Baseline and RAHA at 100, 200, and 500 pairs."><div class="caption">Transfer improves as synthetic image-text capacity grows.</div></div>
        </div>
      </div>
    </section>
    ...
    <section class="band" id="qualitative">
      <div class="wrap">
        ...
        <div class="figure fade"><img src="%%MAIN_QUAL%%" alt="Fig. 1 qualitative synthesized image-text pairs comparing initialization, Baseline, and RAHA."><div class="caption">Fig. 1: Qualitative examples of synthesized image-text pairs. Each group compares initialization, Baseline, and RAHA, showing that RAHA produces more semantically aligned images and text with fewer high-frequency artifacts.</div></div>
      </div>
    </section>
    ...
    <section class="band" id="classification">
      <div class="wrap">
        <div class="section-head fade">
          ...
          <p class="lead">At 100 pairs, RAHA improves top-1 prompted classification on all four benchmarks reported in the appendix. RAHA-distilled 2×2 samples are shown beside the accuracy table, with full per-dataset comparisons below.</p>
        </div>
        <div class="figure-table-grid classification-layout">
          <div class="figure classification-samples fade">
            <div class="classification-samples-grid">
              <div class="classification-sample-cell">
                <div class="qual-label">CIFAR-100</div>
                <div class="media"><img src="res/qual_cifar_raha.png" alt="RAHA-distilled samples on CIFAR-100."></div>
              </div>
              <div class="classification-sample-cell">
                <div class="qual-label">CUB-200-2011</div>
                <div class="media"><img src="res/qual_cub_raha.png" alt="RAHA-distilled samples on CUB-200-2011."></div>
              </div>
              <div class="classification-sample-cell">
                <div class="qual-label">Stanford Cars</div>
                <div class="media"><img src="res/qual_cars_raha.png" alt="RAHA-distilled samples on Stanford Cars."></div>
              </div>
              <div class="classification-sample-cell">
                <div class="qual-label">ImageNet-1K</div>
                <div class="media"><img src="res/qual_imagenet_raha.png" alt="RAHA-distilled samples on ImageNet-1K."></div>
              </div>
            </div>
          </div>
        ...
        <div class="qual-grid classification-qual-grid">
          <div class="figure fade"><div class="qual-label">CIFAR-100</div><div class="media"><img src="%%QUAL_CIFAR%%" alt="CIFAR-100 qualitative distilled samples from the appendix."></div><div class="caption">Distilled data comparison on CIFAR-100. Left: initial samples. Middle: Baseline-distilled. Right: RAHA-distilled.</div></div>
          <div class="figure fade"><div class="qual-label">CUB-200-2011</div><div class="media"><img src="%%QUAL_CUB%%" alt="CUB qualitative distilled samples from the appendix."></div><div class="caption">Distilled data comparison on CUB-200-2011. Left: initial samples. Middle: Baseline-distilled. Right: RAHA-distilled.</div></div>
          <div class="figure fade"><div class="qual-label">Stanford Cars</div><div class="media"><img src="%%QUAL_CARS%%" alt="Stanford Cars qualitative distilled samples from the appendix."></div><div class="caption">Distilled data comparison on Stanford Cars. Left: initial samples. Middle: Baseline-distilled. Right: RAHA-distilled.</div></div>
          <div class="figure fade"><div class="qual-label">ImageNet-1K</div><div class="media"><img src="%%QUAL_IMAGENET%%" alt="ImageNet qualitative distilled samples from the appendix."></div><div class="caption">Distilled data comparison on ImageNet-1K. Left: initial samples. Middle: Baseline-distilled. Right: RAHA-distilled.</div></div>
        </div>
      </div>
    </section>
