...
@torch.no_grad()
def print_results(metrics_dict, title="zero-shot classification results"):
	"""Print a table of classification metrics: Top-1 (%), Top-5 (%), ECE, NLL."""
	import prettytable
	mean_result, std_result = {}, {}
	for k, v in metrics_dict.items():
		key = k.replace("ZSCls/", "") if "ZSCls/" in str(k) else k
		vals = np.asarray(v).reshape(-1)
		mean_result[key] = float(np.mean(vals))
		std_result[key] = float(np.std(vals)) if len(vals) > 1 else 0.0
	...
	results_table.field_names = ["Top-1 (%)", "Top-5 (%)", "ECE", "NLL"]
	if prettytable.__version__ >= "3.16.0":
		results_table.add_divider()
	else:
		results_table.add_row(["-" * 10] * 4)
	nll_mean = mean_result.get("nll", float("nan"))
	nll_std = std_result.get("nll", 0.0)
	results_table.add_row(
		[f"{mean_result['top1']:.2f}", f"{mean_result['top5']:.2f}", f"{mean_result['ece']:.4f}", f"{nll_mean:.4f}"]
	)
	results_table.add_row(
		[f"± {std_result['top1']:.2f}", f"± {std_result['top5']:.2f}", f"± {std_result['ece']:.4f}", f"± {nll_std:.4f}"]
	)
...
def hyperbolic_relevance_matching_loss(
	...
	total = (
		...
		+ float(w_residual_xcov) * residual_dist
		+ float(w_residual_compress) * residual_compress
	)
	return {
		...
		"residual_energy_syn": residual_energy_syn, "total": total,
	}
...
def main(args):
	...
	args_zscls = copy.deepcopy(args)
	args_zscls.dataset = args.zscls_dataset
	if args.zscls_dataset not in ZSCLS_DATASETS:
		raise ValueError(
			f"--zscls_dataset must be one of {sorted(ZSCLS_DATASETS)}. Got: {args.zscls_dataset}"
		)
	_, zscls_testloader, _, zscls_test_dataset = get_zscls_dataloaders(args_zscls)
	...
	if args.wandb:
		wandb.init(
			...
			name=f"{args.name}_{args.zscls_dataset}_N{args.num_queries}_seed{args.seed}_{make_timestamp()}",
			config=args
		)
	else:
	...
	best_zs_top1 = -float("inf")
	best_eval_iter = -1
	best_zs_metrics = None
	best_zs_stds = None
	early_stop_patience = 100
	model_init_times = []
	data_init_times = []
	distillation_times = []
	wall_clock_tracker = WallClockTracker(metric_name="top1")

	for it in tqdm(range(1, args.Iteration + 1)):
		clean_cache()
		...
		if it > 0 and it % args.eval_it == 0:
			...
			for it_eval in range(args.num_eval):
				...
				_, cls_metrics = evaluate_synset_zscls(
					it_eval=it,
					net=net_eval,
					images_train=image_syn_eval,
					texts_train=text_syn_eval,
					mask_train=mask_syn_eval,
					cls_loader=zscls_testloader,
					cls_dataset=zscls_test_dataset,
					args=args,
				)
				for k, v in cls_metrics.items():
					multi_eval_aggr_result[k].append(v)

			top1_mean = float(np.mean(multi_eval_aggr_result["ZSCls/top1"]))
			top5_mean = float(np.mean(multi_eval_aggr_result["ZSCls/top5"]))
			ece_mean = float(np.mean(multi_eval_aggr_result["ZSCls/ece"]))
			nll_mean = float(np.mean(multi_eval_aggr_result["ZSCls/nll"]))
			n_top1 = len(multi_eval_aggr_result["ZSCls/top1"])
			top1_std = float(np.std(multi_eval_aggr_result["ZSCls/top1"])) if n_top1 > 1 else 0.0
			top5_std = float(np.std(multi_eval_aggr_result["ZSCls/top5"])) if n_top1 > 1 else 0.0
			ece_std = float(np.std(multi_eval_aggr_result["ZSCls/ece"])) if n_top1 > 1 else 0.0
			nll_std = float(np.std(multi_eval_aggr_result["ZSCls/nll"])) if n_top1 > 1 else 0.0
			print(f"[zscls] iter={it} top1={top1_mean:.2f}±{top1_std:.2f}% top5={top5_mean:.2f}±{top5_std:.2f}% ECE={ece_mean:.4f}±{ece_std:.4f} NLL={nll_mean:.4f}±{nll_std:.4f}")

			wandb.log(
				{
					"Eval/ZSCls/top1": top1_mean,
					"Eval/ZSCls/top5": top5_mean,
					"Eval/ZSCls/ece": ece_mean,
					"Eval/ZSCls/nll": nll_mean,
					"Eval/Std/ZSCls/top1": top1_std,
					"Eval/Std/ZSCls/top5": top5_std,
					"Eval/Std/ZSCls/ece": ece_std,
					"Eval/Std/ZSCls/nll": nll_std,
				},
				step=it,
			)

			wc = wall_clock_tracker.stamp_eval(it=it, value=top1_mean)
			wandb.log(
				{
					"WallClock/current_zs_top1": top1_mean,
					"WallClock/elapsed_s": wc["elapsed_s"],
				},
				step=it,
			)

			if top1_mean > best_zs_top1:
				best_zs_top1 = top1_mean
				best_eval_iter = it
				best_zs_metrics = {
					"top1": list(multi_eval_aggr_result["ZSCls/top1"]),
					"top5": list(multi_eval_aggr_result["ZSCls/top5"]),
					"ece": list(multi_eval_aggr_result["ZSCls/ece"]),
					"nll": list(multi_eval_aggr_result["ZSCls/nll"]),
				}
				best_zs_stds = {"top1": top1_std, "top5": top5_std, "ece": ece_std, "nll": nll_std}
				wandb.log(
					{
						"BestEval/ZSCls/top1": top1_mean,
						"BestEval/ZSCls/top5": top5_mean,
						"BestEval/ZSCls/ece": ece_mean,
						"BestEval/ZSCls/nll": nll_mean,
						"BestEval/Std/ZSCls/top1": top1_std,
						"BestEval/Std/ZSCls/top5": top5_std,
						"BestEval/Std/ZSCls/ece": ece_std,
						"BestEval/Std/ZSCls/nll": nll_std,
						"BestEval/iter": it,
						"WallClock/time_to_best_s": wc["best_time_s"],
						"WallClock/best_zs_top1": wc["best_val"],
					},
					step=it,
				)
				...
				print(f"[BEST zscls] it={it} top1={top1_mean:.2f}% top5={top5_mean:.2f}% ECE={ece_mean:.4f} NLL={nll_mean:.4f} saved to {best_path}")
				results_table = print_results(
					best_zs_metrics,
					title=f"<best> zero-shot classification on {args.zscls_dataset} at iteration {it}",
				)
				print(results_table)

			eval_log_dict = {
				"Eval/Mean/ZSCls/top1": top1_mean,
				"Eval/Mean/ZSCls/top5": top5_mean,
				"Eval/Mean/ZSCls/ece": ece_mean,
				"Eval/Mean/ZSCls/nll": nll_mean,
				"Eval/Std/ZSCls/top1": top1_std,
				"Eval/Std/ZSCls/top5": top5_std,
				"Eval/Std/ZSCls/ece": ece_std,
				"Eval/Std/ZSCls/nll": nll_std,
				"BestEval/Mean/ZSCls/top1": best_zs_top1,
				"BestEval/iter": best_eval_iter,
			}
			if best_zs_stds is not None:
				eval_log_dict["BestEval/Std/ZSCls/top1"] = best_zs_stds["top1"]
				eval_log_dict["BestEval/Std/ZSCls/top5"] = best_zs_stds["top5"]
				eval_log_dict["BestEval/Std/ZSCls/ece"] = best_zs_stds["ece"]
				eval_log_dict["BestEval/Std/ZSCls/nll"] = best_zs_stds["nll"]
			...
			if best_eval_iter >= 0 and (it - best_eval_iter) >= early_stop_patience:
				print(
					...
					"(no improvement in zs_top1 for %d iterations; "
					"best top1 = %.2f%% at iteration %d)"
					% (it, early_stop_patience, best_zs_top1, best_eval_iter)
				)
				break
		...
		total_model_init_time += model_init_time
		...
		total_distill_time += distill_time

		avg_model_init_time = total_model_init_time / it
		avg_distill_time = total_distill_time / it
	...
	print(
		f"[WallClock] best {wall_clock_tracker.metric_name}={wall_clock_tracker.best_val:.2f}% | "
		f"time_to_best={time_to_best_s} | time_to_target={time_to_target_s}"
	)
	print(f"[WallClock] history saved -> {json_path}")

	if best_zs_metrics is not None:
		final_means = {k: float(np.mean(v)) for k, v in best_zs_metrics.items()}
		final_stds = {k: float(np.std(v)) if len(v) > 1 else 0.0 for k, v in best_zs_metrics.items()}
		wandb.log(
			{
				"Final/ZSCls/top1_mean": final_means["top1"],
				"Final/ZSCls/top1_std": final_stds["top1"],
				"Final/ZSCls/top5_mean": final_means["top5"],
				"Final/ZSCls/top5_std": final_stds["top5"],
				"Final/ZSCls/ece_mean": final_means["ece"],
				"Final/ZSCls/ece_std": final_stds["ece"],
				"Final/ZSCls/nll_mean": final_means["nll"],
				"Final/ZSCls/nll_std": final_stds["nll"],
			},
			step=best_eval_iter,
		)
		results_table = print_results(
			best_zs_metrics,
			title=f"Final best zero-shot classification on {args.zscls_dataset} (mean ± std)",
		)
		print(results_table)
...
if __name__ == '__main__':
	...
	parser.add_argument('--dataset', type=str, default='flickr', help='dataset for distillation (retrieval)')
	parser.add_argument(
		'--zscls_dataset',
		type=str,
		default='imagenet',
		choices=sorted(ZSCLS_DATASETS),
		help='zero-shot prompted classification eval dataset. Supported: %(choices)s',
	)
	...
	BASE_DIR = os.environ.get("MDD_BASE_DIR", "/mnt/sdk/adv_datasets")
	ZSCLS_BASE = os.environ.get("ZSCLS_BASE_DIR", BASE_DIR)


	args.dataset = args.zscls_dataset

	args.zscls_base_dir = ZSCLS_BASE
